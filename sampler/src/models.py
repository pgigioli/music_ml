...
class Segmenter1d(nn.Module):
    def __init__(self, h_dim=512, sigmoid=True, depths=(64, 64, 128, 128, 256, 256), time_reduction=0):
        ...
        if time_reduction > 6:
            raise Exception('time reduction must be less than or equal to number of layers (6)')
        
        time_pooling_kernel = [1]*6
        time_pooling_stride = [1]*6
        
        for i in range(time_reduction):
            time_pooling_kernel[i] = 2
            time_pooling_stride[i] = 2
        ...
        self.encode = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(depths[0]),
            nn.MaxPool2d((2, time_pooling_kernel[0]), stride=(2, time_pooling_stride[0])),
            
            nn.Conv2d(depths[0], depths[1], (3, 9), padding=(1, 4), stride=1),
            nn.ReLU(),
            nn.BatchNorm2d(depths[1]),
            nn.MaxPool2d((2, time_pooling_kernel[1]), stride=(2, time_pooling_stride[1])),
            
            nn.Conv2d(depths[1], depths[2], (3, 9), padding=(1, 4), stride=1),
            nn.ReLU(),
            nn.BatchNorm2d(depths[2]),
            nn.MaxPool2d((2, time_pooling_kernel[2]), stride=(2, time_pooling_stride[2])),
            
            nn.Conv2d(depths[2], depths[3], (3, 9), padding=(1, 4), stride=1),
            nn.ReLU(),
            nn.BatchNorm2d(depths[3]),
            nn.MaxPool2d((2, time_pooling_kernel[3]), stride=(2, time_pooling_stride[3])),
            
            nn.Conv2d(depths[3], depths[4], (3, 9), padding=(1, 4), stride=1),
            nn.ReLU(),
            nn.BatchNorm2d(depths[4]),
            nn.MaxPool2d((2, time_pooling_kernel[4]), stride=(2, time_pooling_stride[4])),
            
            nn.Conv2d(depths[4], depths[5], (3, 9), padding=(1, 4), stride=1),
            nn.ReLU(),
            nn.BatchNorm2d(depths[5]),
            nn.MaxPool2d((2, time_pooling_kernel[5]), stride=(2, time_pooling_stride[5])),
    ...
    def forward(self, x):
        ...
        if self.time_reduction != 0:
            outputs = F.interpolate(outputs.unsqueeze(1), (x.size(-1))).squeeze(1)
